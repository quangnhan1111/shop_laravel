...
class CartController extends Controller
{
    ...
    public function update_cart(Request $request){
        ...
        $cart = Session::get('cart');
        if($cart==true){
            // key chính là session id gửi kèm name="cart_qty[{{$cart['session_id']}}]"
            foreach($data['cart_qty'] as $key => $qty){
                // session chính là thứ tự của cart
                foreach($cart as $session => $val){
                    if($val['session_id']==$key){
                        if($qty < $cart[$session]['product_quantity'])
                        // update cart mang session thuocc tính qty
                        $cart[$session]['product_qty'] = $qty;
                        else{
                            return redirect()->back()->with('message','số lượng đéo đủ!!');
                        }
                    }
                }
            }
